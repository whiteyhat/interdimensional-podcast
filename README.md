...
- `/studio?broadcast=1&autostart=1` is the **broadcast view**: no chrome, the stage filling a 16:9 viewport with the lower-third, captions and the price bug, and a hover-only control strip. Point an OBS browser source at it.
...
The buffer smooths playback but adds a delay to paid requests. Generation can still stall or be rejected. Captions show the script; generated speech and voices can vary. Stop ends new submissions, while jobs already sent may finish. Reloading the studio keeps unaired paid requests but clears the session.
...
4. **Stream.** On the coin's pump.fun page choose *Start livestream* and the RTMP option (the creator wallet needs streaming enabled) to get a stream URL and key. In OBS add a browser source at `http://127.0.0.1:3212/studio?broadcast=1&autostart=1` at 1920×1080 with audio controlled by OBS, and send the same stream to pump.fun, to X live and to a Cloudflare Stream live input. Paste the Cloudflare Stream player URL into `STREAM_EMBED_URL` and your X broadcast into `X_LIVE_URL` so the public site can show the stream. X Spaces takes the audio through a virtual audio device.
...
```sh
node --test tests/engine.test.mjs tests/gestures.test.mjs tests/video-render.test.mjs tests/topics.test.mjs tests/gnews.test.mjs tests/requests.test.mjs tests/coin.test.mjs tests/pumpchat.test.mjs
npx tsc --noEmit
npx oxlint app lib scripts hooks/use-poll.ts hooks/use-coin.ts hooks/use-config.ts hooks/use-requests.ts components/player.tsx components/ticker.tsx components/feed-panel.tsx components/coin-card.tsx components/coin-bug.tsx components/chat-panel.tsx components/interact-card.tsx components/wallet.tsx components/requests-list.tsx components/stream-embed.tsx components/public-site.tsx components/studio.tsx
npm run build
```
