...
Checkout is **disabled by default**. The implementation and local rehearsal are complete; the live staging rehearsal and production rollout are separate remaining release gates. No production transfers, refunds, or deployments were performed for this work.
...
| Refund reserves | `SPONSOR_REFUND_SECRET_KEY` | Separate server-only key; fund each enabled asset and SOL for fees |
...
| Reconciliation service | `SPONSOR_ORIGIN`, `SPONSOR_RECONCILE_TOKEN` | Independent payment recovery, rescheduling and refunds |
...
The refund signer must be a different key from the treasury and must hold the original assets, plus SOL. Checkout checks uncommitted refund liquidity for the exact attempted purchase. A default USDC availability check can pass while a larger product exceeds the reserves; its quote is then refused before signing. The buyer pays the original checkout network fee. Refunds return the exact transfer amount; the service pays refund fees.

`SPONSOR_USDC_MINT` is for a staging test mint only. SOL/FROGCLENCH quotes use actual Jupiter prices; an unpriced devnet token correctly stays unavailable. Rehearse its missing-price state on devnet and use the deterministic injected-chain integration suite for its priced, exact-amount settlement/refund cases. Do not silently substitute a fixed price on a production deployment. A fully live test-token pricing fixture remains part of staging setup if all three assets must transact on devnet.

The producer discovers media readiness and advertises compatible products. Director mode does not advertise sponsorship delivery and cannot sell this catalog. Cap sales require a qualified media worker, R2 storage, a live clip producer, payment/refund readiness, and a free reservation for the selected host. There is one purchased or pending reservation per host. Existing attempts remain recoverable while inventory is full; a late discovered payment remains a recorded obligation and can queue behind a newer reservation or be refunded. Only one cap can air per host at a time.
...
A frontend timer never cancels a possible on-chain payment. Recovery scans beyond failed reference transactions, retains incomplete scan cursors, and records duplicate or late payments for reconciliation. An ambiguous broadcast keeps the same signed transaction and attempt. Refund execution similarly persists its signed transaction before sending; an unresolved expired refund is held for archival/operator reconciliation rather than issuing a potentially duplicate replacement.

The studio console shows orders, active wardrobe, delivery failures and refund states. A placement completes from playback, never merely from successful generation. Caps start their ten-minute clock at the first verified appearance, accrue only healthy broadcast time, and need at least six clear appearances plus introduction and callback. The wardrobe revision is pinned into each render/cache identity; gestures that obstruct the cap are deferred. Paid dialogue, including wearable mentions and legacy requests, reserves at most one exchange per 120 seconds of program time.

On playback failure, the producer revokes the affected media, sends durable pause intents, and withholds lease renewal until invalidation is acknowledged. A delayed play promise cannot restart revoked media. Pauses can be acknowledged even after heartbeat expiry, with exact lease ownership still enforced. Unfinished orders automatically reschedule with a bounded retry policy; customers can also reschedule or request a full refund before airing or while paused. The refund transition atomically stops future fulfillment before funds are sent.

Inspect `sponsor_refunds` with `status='blocked'` and `sponsor_payments` with `is_late=1` during operations. A funded reserve outage leaves a durable refund obligation visible in the console and receipt. Restore liquidity and let reconciliation retry. Preserve signed refund evidence when diagnosing ambiguous outcomes.
...
Keep `SPONSOR_ENABLED=false` until a staging environment and funded test refund wallet are selected. In that environment, build and run both service containers, apply migrations, and validate:
...
- Producer restart, stale heartbeat, stalled/decode-failed media, interrupted cap placement, rescheduling, cancel-versus-playback races and original-asset refunds.
- Unavailable refund liquidity and ambiguous refund broadcasts remain recoverable without duplicate payouts.
