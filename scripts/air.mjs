...
const BOX_ONLY = ['AIR_MAX_MINUTES', 'AIR_HEIGHT', 'AIR_FPS', 'AIR_BITRATE', 'AIR_ALERT_WEBHOOK'];

async function boxUrl() {
  const saved = await devVar('AIR_URL');
  if (!saved) throw Error('The box has no address yet. Run: node scripts/air.mjs setup');
  return saved.replace(/\/$/, '');
}

async function box(path, { method = 'GET', body } = {}) {
  const token = await devVar('STUDIO_TOKEN');
  if (!token) throw Error(`Set STUDIO_TOKEN in ${VARS}; it is what the box trusts.`);
  const response = await fetch(`${await boxUrl()}${path}`, {
    method,
  ...
  if (!response.ok) throw Error(data.error || `The box answered ${response.status}.`);
  return data;
}
...
async function setup(args) {
  ...
  if (!studio.FAL_KEY) throw Error(`Set FAL_KEY in ${VARS}: it is what generates the show.`);
  if (!studio.STUDIO_TOKEN)
    throw Error(`Set STUDIO_TOKEN in ${VARS}: without it the box would take orders from anyone.`);
  if (rehearsal) delete studio.INTERACT_ORIGIN;
  else if (!studio.INTERACT_ORIGIN)
    console.warn('Warning: INTERACT_ORIGIN is not set, so the five-dollar seat stays closed.');
  ...
  const box = (await devVar('AIR_STUDIO_ID')) || 'railway';
  if (mine && mine === box)
    throw Error(
  ...
  const settings = { ...studio, STUDIO_ID: box, ...(await devVars(BOX_ONLY)) };
  settings.AIR_MAX_MINUTES ||= '1440';
  console.log(`The box calls itself "${box}"${mine ? `; this machine is "${mine}"` : ''}.`);
  ...
  const input = (await devVar('AIR_RTMP_INPUT')) || (await devVar('CF_LIVE_INPUT_ID'));
  if (input) {
    ...
    console.warn('Warning: no live input configured, so the show will run without being streamed.');
  }

  const ids = await railway.target();
  await railway.setVariables(ids, settings);
  await railway.configure(ids, {
...
async function deploy() {
  ...
  console.log(`Building ${started.megabytes} MB as deployment ${started.id.slice(0, 8)}.`);
  let last = '';
  for (let i = 0; i < 150; i++) {
    ...
    if (['FAILED', 'CRASHED', 'REMOVED'].includes(state.status)) {
      console.error(`\nThe build ${state.status.toLowerCase()}. Last output:\n`);
      for (const line of (await railway.logs(started.id, 'build', 40)).slice(-25)) console.error(`  ${line}`);
      process.exit(1);
    }
  }
  console.error('The build is still going after 25 minutes; check Railway.');
}

async function on(args) {
  const flag = args.indexOf('--minutes');
  const minutes = flag === -1 ? undefined : Number(args[flag + 1]);
  const result = await box('/air/on', { method: 'POST', body: minutes ? { minutes } : {} });
  console.log(result.already ? 'Already on air.' : 'On air.');
  report(result);
  if (!result.already) console.log('\nThe first frame is about 90 seconds away while the reserve builds.');
}
...
function report(status) {
  ...
  console.log(`  show      ${show.phase ?? 'not started'}${show.error ? ` — ${show.error}` : ''}`);
  console.log(`  buffer    ${show.buffered ?? 0}s of footage ready`);
  console.log(`  aired     ${show.aired ?? 0} clips, ${show.stalls ?? 0} stalls, ${status.reloads ?? 0} reloads`);
  console.log(`  encoder   ${status.encoder} (${status.encoderRestarts} restarts), ${status.video}`);
  if (status.minutesLeft !== null && status.minutesLeft !== undefined)
    console.log(`  ends in   ${status.minutesLeft} minutes`);
  console.log(`  box uses  ${status.memoryMb} MB, up ${Math.round((status.uptimeSeconds ?? 0) / 60)} minutes`);
}

async function status() {
  report(await box('/air/status'));
  console.log('\nCloudflare:');
  const child = spawn(process.execPath, ['scripts/stream.mjs', 'status'], { stdio: 'inherit' });
...
async function logs(args) {
  ...
  console.log(`Deployment ${found.id.slice(0, 8)} (${found.status.toLowerCase()}), ${kind} log:\n`);
  for (const line of await railway.logs(found.id, kind, 80)) console.log(`  ${line}`);
}
...
if (!run) {
  console.error(`Usage: node scripts/air.mjs <${Object.keys(commands).join('|')}>`);
  process.exit(1);
}
try {
  await run(rest);
